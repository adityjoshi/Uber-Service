...
func (r *RideRepository) FindByDriverId(ctx context.Context, driverID string) ([]*model.Ride, error) {
	query := `
		SELECT id, rider_id, driver_id, pickup_latitude, pickup_longitude, pickup_address, drop_latitude, drop_longitude, drop_address, status, estimated_fare, actual_fare, created_at, updated_at, started_at, completed_at
	FROM rides WHERE driver_id = $1 ORDER BY created_at DESC`
	rows, err := r.db.Query(ctx, query, driverID)
	if err != nil {
		return nil, fmt.Errorf("repository: find rides by driver: %w", err)
	}
	defer rows.Close()
